    ...
    try {
      const targetCommand = commands.find((cmd) => {
        if (!cmd?.commands || !Array.isArray(cmd.commands)) {
          errorLog(
            `Error en el comando de tipo "${type}": La propiedad "comandos" debe existir para ser una ["array"] ¡con los nombres de los comandos! Archivo incorrecto: ${cmd.name}.js`
          );
      ...
      console.error(`Error al procesar comandos de tipo "${type}":`, error);
    }
  }
