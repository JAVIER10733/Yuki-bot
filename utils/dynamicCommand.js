...
exports.dynamicCommand = async (paramsHandler) => {
  ...
  if (isActiveAntiLinkGroup(remoteJid) && isLink(fullMessage)) {
    ...
    if (!(await isAdmin({ remoteJid, userJid, socket }))) {
      ...
      await sendReply(
        "¡Anti-enlace activado! Te han eliminado por enviar un enlace!"
      );
  ...
  if (!isActiveGroup(remoteJid) && command.name !== "on") {
    await sendWarningReply(
      "¡Este grupo está deshabilitado! ¡Pídele al propietario del grupo que active el bot!"
    );
  ...
  try {
    ...
    if (error instanceof InvalidParameterError) {
      await sendWarningReply(`Parámetros no válidos! ${error.message}`);
